...
SPEED FIX: Uses cached test feature maps and cached gradients.
For random CAVs, uses fast dot product attribution (no full IG)
since we only need relative comparison against real CAVs.
Real CAV attributions are loaded from the global run cache.
...
TTEST_DIR    = os.path.join(PROJECT_ROOT, "outputs", "vtcav_ttest_run2")
...
# For t-test we need relative comparison, not
# absolute Visual-TCAV scores. Using fast
# dot product attribution is statistically
# equivalent for significance testing.
# ─────────────────────────────────────────────
def fast_attribution(test_fmaps_pooled, direction):
    """
    Fast attribution = mean dot product of pooled feature maps with CAV.
    test_fmaps_pooled: (N, C) pooled feature maps
    direction: (C,) CAV direction
    Returns: list of N scalar scores
    """
    direction_np = direction.numpy() if hasattr(direction, 'numpy') else direction
    scores = test_fmaps_pooled @ direction_np  # (N,)
    return scores.tolist()


def load_or_compute_test_fmaps(class_name, layer_name):
    """Load cached test feature maps or compute and cache them."""
...
for concept_name, concept_folder in CONCEPTS.items():
    ...
    for layer_name in LAYERS:
        ...
        pos_cache_path = os.path.join(
            CACHE_DIR,
            f"cav_{safe_name}_{MAX_EXAMPLES}_neg_{N_CAV_RUNS}runs_{layer_name}.joblib"
        )

        # Re-extract positive feature maps for computing real directions
        gen = ImageActivationGenerator(
            model_wrapper=wrapper,
        ...
        # ── Compute mean real CAV direction (20 runs) ──────────
        real_directions = []
        ...
        # ── Compute 50 random CAV directions ───────────────────
        random_directions = []
        ...
        # ── For each class: compute real and random scores ──────
        for class_name in CLASSES:

            # Load cached test feature maps
            try:
            ...
            # Pool test feature maps: (N, 7, 7, 2048) → (N, 2048)
            test_pooled = tf.reduce_mean(test_fmaps, axis=(1, 2)).numpy()

            # Real attribution scores using mean direction
            real_scores = fast_attribution(test_pooled, mean_real_direction)

            # Random attribution scores — one mean per random CAV
            random_mean_scores = []
...
# ─────────────────────────────────────────────
# 7. SUMMARY TABLES — all classes at post_relu
# ─────────────────────────────────────────────
print(f"\n{'='*70}")
print("SUMMARY — post_relu layer")
print(f"{'='*70}")
...
# ─────────────────────────────────────────────
# 8. PLOTS — Lucieri et al. Figure 4 style
# ─────────────────────────────────────────────
