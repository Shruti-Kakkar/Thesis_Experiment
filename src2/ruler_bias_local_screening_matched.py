"""
ruler_bias_local_screening_matched.py
Same as ruler_bias_local_screening.py, but scores against the CORRECTED
'ruler_present' CAV via ruler_bias_common_matched. Run
ruler_bias_finalize_matched.py afterwards for summary statistics and
top-outlier heatmaps.
...
memory) before the next chunk starts, e.g.:
    ...
    python ruler_bias_local_screening_matched.py --start 0  --end 10
    python ruler_bias_local_screening_matched.py --start 10 --end 20
    python ruler_bias_local_screening_matched.py --start 20 --end 30
    python ruler_bias_local_screening_matched.py --start 30 --end 40
    python ruler_bias_local_screening_matched.py --start 40 --end 50
...
rows (ruler_bias_finalize_matched.py does not deduplicate).

post_relu ONLY, same as the confounded run -- keeps this fast and
directly comparable.
...
from ruler_bias_common_matched import build_targets, make_local_tcav, RESULTS_CSV
...
print(f"Screening images [{args.start}:{end}] of {len(targets)} total, "
      f"post_relu only, corrected concept set\n")
