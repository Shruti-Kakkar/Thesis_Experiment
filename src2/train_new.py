...
CHANGES vs. train.py (Run 1):
  1. Aspect-ratio-preserving resize with zero-padding (tf.image.resize_with_pad),
     ...
     Rationale: Nigar et al. (2022) explicitly pad the short side before resizing
     to 224x224 to avoid destroying shape information -- relevant here since
     asymmetry is one of the seven-point checklist concepts used downstream in
     Visual-TCAV.
  2. Added random rotation to the augmentation pipeline (dermoscopic images have
     no canonical orientation), following the heavier augmentation used in
     Gessert et al. (2019).
  Color constancy (Shades of Gray) and cropping of raw uncropped images were
  intentionally NOT added here -- flagged separately for discussion given their
  potential interaction with color-based concepts (e.g. blue-whitish veil).
"""
...
# Separate tags: model checkpoints/final model use MODEL_TAG,
# all output artifacts (plots, reports) use OUTPUT_TAG.
MODEL_TAG   = "_new"
OUTPUT_TAG  = "_2New"
...
# 5. DATA PIPELINE
# ─────────────────────────────────────────────
# Random rotation layer (small-angle, reflect padding to avoid introducing
# artificial black borders that could confound the ruler-artifact analysis).
random_rotation = tf.keras.layers.RandomRotation(
    factor=0.05,          # ~ +/-18 degrees
    fill_mode='reflect')
...
def load_and_preprocess(filepath, label, augment=False):
    ...
    # --- CHANGE 1: aspect-ratio-preserving resize with zero-padding ---
    # Replaces the previous tf.image.resize([IMG_SIZE, IMG_SIZE]), which
    # stretched/squashed images regardless of original aspect ratio.
...
plt.title('Confusion Matrix — ResNet50V2 on ISIC 2019 (new preprocessing)')
