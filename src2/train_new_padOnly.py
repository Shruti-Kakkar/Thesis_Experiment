...
ABLATION vs. train_new.py:
  Isolates the aspect-preserving resize+pad change from the rotation-augmentation
  change, since the combined run (train_new.py) showed a net accuracy REGRESSION
  vs. Run 1 (64% -> 60%) and the two changes were confounded. This script keeps
  ONLY the resize_with_pad change and drops rotation, so any accuracy delta here
  can be attributed to the resize change alone.

CHANGES vs. train.py (Run 1):
  1. Aspect-ratio-preserving resize with zero-padding (tf.image.resize_with_pad),
     replacing the direct resize-to-224x224 that distorted lesion shape.
  2. EarlyStopping patience increased 5 -> 8. The rotation-augmented run showed
     val_accuracy still climbing when val_loss-based early stopping fired
     (restored weights from epoch 21 while val_accuracy kept improving through
     epoch 26). Giving more patience here in case padding-only convergence is
     also slower than Run 1's.
  NOT included: rotation augmentation, color constancy, border-cropping --
  all deliberately held out to keep this a single-variable test.
"""
...
# Distinct tags so this run doesn't overwrite the rotation+padding run
# (models2/*_new.keras, outputs2/*_2New.*) already saved.
MODEL_TAG   = "_padonly"
OUTPUT_TAG  = "_PadOnly"
...
def load_and_preprocess(filepath, label, augment=False):
    ...
    # Aspect-ratio-preserving resize with zero-padding (kept from train_new.py)
    img = tf.image.resize_with_pad(img, IMG_SIZE, IMG_SIZE)

    img = tf.keras.applications.resnet_v2.preprocess_input(img)

    if augment:
        # Same augmentation as Run 1 -- NO rotation here (ablation control)
...
plt.title('Confusion Matrix — ResNet50V2 on ISIC 2019 (padding only, no rotation)')
