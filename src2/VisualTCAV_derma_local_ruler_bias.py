...
visually check where the model's ruler-related attribution actually
lands -- directly on physical ruler markings, or elsewhere.

KEY DESIGN DIFFERENCE from the clinical-concepts Local script: images are
picked from your ground-truth-sorted Normal_Ruler/Clean folders, not the
test set -- because Global TCAV's conv5_block1_out vs post_relu
disagreement (NV >> MEL early, MEL > NV late) could be explained by not
knowing whether the *test* images used actually had rulers. Here, ruler
presence is a known, controlled variable.

4 categories, 2 images each (8 total):
  - MEL + Normal_Ruler  (does attribution land on the physical ruler?)
  - NV  + Normal_Ruler  (same question, benign case)
  - MEL + Clean         (baseline: does attribution correctly drop near 0?)
  - NV  + Clean         (same baseline, benign case)

CAVEAT (real, worth keeping in your writeup): these are TRAINING images
(the ones you manually sorted), not held-out test images -- the model
has seen them during training. Necessary tradeoff, since only these have
confirmed ground-truth ruler labels.

Reuses the SAME model, MODEL_TAG, and ruler_present CAV cache as the
Global ruler-bias run (n_cav_runs=20 must match exactly to hit that
cache) -- should not need to recompute the CAV itself.
...
# ─────────────────────────────────────────────
# 0. MODEL VARIANT TAG — same as the Global ruler-bias run
# ─────────────────────────────────────────────
...
CONCEPT_DIR = os.path.join(PROJECT_ROOT, "concept_images_ruler")  # same as Global script
...
# ─────────────────────────────────────────────
# 2. SYMLINKS — same as Global script, idempotent (safe if already present)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 3. MODEL FOLDER STRUCTURE — reuses the Global run's symlink
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 5. GROUND-TRUTH ID -> CLASS LOOKUP (same logic as ruler_prevalence_by_class.py)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 6. PICK EXAMPLES: 2 each of MEL+ruler, NV+ruler, MEL+clean, NV+clean
# ─────────────────────────────────────────────
...
for subfolder, true_class, has_ruler, fname in example_images:
    ...
    print(f"\n{'='*60}")
    print(f"LocalVisualTCAV — image: {rel_image_path} "
    ...
    local_tcav = LocalVisualTCAV(
        ...
        model=Model(
            ...
            max_examples=200,   # must match Global script's value -- cache key depends on it
            resize_mode='pad',
        ),
    ...
    # n_cav_runs=20 MUST match the Global ruler-bias script's n_cav_runs
    ...
    def _save_instead_of_show():
        ...
        fname_out = os.path.join(
            RESULTS_DIR,
            f"local_{image_label}_{true_class}_{ruler_tag}_{safe_concept}.png"
        )
